...
public class WebSecurityConfig {
    ...
    // 로그인 로직에서 제외할 폴더를 지정한다
    // /static/** : 정적 폴더 → 아래 /css, /js, /img 
    @Bean
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		...
		// .authorizeHttpRequests() : 새로운 함수 : HttpRequests 를 통해서 어떤 일을 할수있는지 정해놓음
		.authorizeHttpRequests((auth) ->
				auth
		//			.requestMatchers(PathRequest.toH2Console()).permitAll()
					// "/login", "/signup", "/user" 는 요청인가 없이 접근허용
					//  로그인    회원가입    회원등록 기능
					//  → 위 3개는 세션에서 빼는 것(시큐리티가 로그인 자동으로 체크하는데, 거기에서 제외)
					.requestMatchers("/login", "/signup", "/user").permitAll()
					.anyRequest().authenticated() // 위 3개 외 나머지 요청은 인증 필요
		)
		.formLogin((formLogin) ->
				formLogin
						.loginPage("/login")		    // 로그인할 페이지 경로				
						.defaultSuccessUrl("/")  		// 로그인 성공시 이동할 페이지(로그인 성공시 경로)
		) // 로그인처리
		.logout((logout) ->
